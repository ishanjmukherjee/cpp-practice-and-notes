...
- The reason an argument must be passed by reference into a copy constructor is [that]([url](https://stackoverflow.com/a/2685871)) "if it's not by reference, it's by value. To do that you make a copy, and to do that you call the copy constructor. But to do that, we need to make a new value, so we call the copy constructor, and so on..."
